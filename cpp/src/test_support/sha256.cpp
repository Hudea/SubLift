...
// Compact SHA-256 (public-domain style). Used by parity harness only so we do
// not pull OpenSSL / CommonCrypto into the public core.
...
std::array<std::uint8_t, 32> sha256_raw(const std::uint8_t* data, std::size_t len) {
...
std::string sha256_prefixed(const std::uint8_t* data, std::size_t len) {
  const auto digest = sha256_raw(data, len);
