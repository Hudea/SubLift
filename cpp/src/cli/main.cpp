...
void print_usage(const char* prog) {
  std::fprintf(stderr,
               ...
               "select an implementation. Missing engine/capability fails closed.\n"
               "\n"
               "extract options:\n"
...
[[nodiscard]] int run_extract(const ExtractArgs& args) {
  if (auto env_err = sublift::cli::product_runtime_env_error_from_process()) {
  ...
  auto worker = sublift::cli::resolve_worker_from_process(args.worker);
  if (!worker.has_value()) {
    if (args.worker.has_value()) {
      std::fprintf(stderr, "Error: sublift_worker not found: %s\n",
                   args.worker->c_str());
    } else {
      std::fprintf(stderr,
  ...
  try {
    ...
    const std::string engines_summary = join_engines(bye.engines);
    if (std::find(bye.engines.begin(), bye.engines.end(), args.engine) ==
        bye.engines.end()) {
      std::fprintf(stderr,
                   "Error: engine '%s' is unavailable on this Native worker "
                   "(available: [%s]). No fallback.\n",
                   args.engine.c_str(), engines_summary.c_str());
    ...
    const auto [srt_body, written_count] = sublift::cli::format_srt(entries);
    {
...
int main(int argc, char** argv) {
  ...
  if (cmd == "extract") {
    ...
    const auto parsed = sublift::cli::parse_extract_args(args);
    ...
    return run_extract(parsed.args);
  }
