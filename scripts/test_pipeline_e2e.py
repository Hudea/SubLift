...
from sublift.config import Config, SignatureConfig, ChangePointConfig
...
def main() -> None:
    ...
    lines.append(f"采样率 {config.sample_fps}fps  检测到 {len(entries_window)} 段  实际 {len(srt_window)} 段")
    lines.append("=" * 100)
    ...
    for i, entry in enumerate(entries_window):
        match, ov, matched = best_match(entry, srt_window)
