...
Phase 4.1 原本仅在 `feat-042` 已完成、上述硬门全部有实际证据、文档同步且仓库可通过标准
启动路径时标记 done。实际实验未满足吞吐硬门，已归档为**未采纳的负向性能证据**；不要在
该功能上继续调参。后续性能工作转入 [Phase 4.2 OCR 内部性能归因](phase4.2-ocr-performance-attribution.md)。
